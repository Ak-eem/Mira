...
const isRecord = (value: unknown): value is R => typeof value === "object" && value !== null;
const records = (value: unknown): R[] => (Array.isArray(value) ? value.filter(isRecord) : []);

function validSignature(header: string | null, body: string) {
...
function inbound(payload: unknown) {
  const result: { id: string; from: string; text: string; phoneId: string; raw: R }[] = [];
  ...
  for (const entry of records(payload.entry)) {
    for (const change of records(entry.changes)) {
      ...
      const phoneId = typeof metadata?.phone_number_id === "string" ? metadata.phone_number_id : "";
      for (const msg of records(value?.messages)) {
        const text = isRecord(msg.text) && typeof msg.text.body === "string" ? msg.text.body : "";
        if (typeof msg.id === "string" && typeof msg.from === "string" && phoneId) {
          result.push({ id: msg.id, from: msg.from, text, phoneId, raw: msg });
        }
      }
    }
  }
...
export async function POST(request: NextRequest) {
  ...
  if (!validSignature(request.headers.get("x-hub-signature-256"), raw)) {
  ...
  const messages = inbound(payload);
  if (messages.some((item) => item.text.length > MAX_MESSAGE_LENGTH)) {
    return NextResponse.json(
      { error: `WhatsApp messages must be ${MAX_MESSAGE_LENGTH} characters or fewer.` },
      { status: 400 },
    );
  }
  ...
  try {
    for (const item of messages) {
      const limits = await Promise.all([
        ...
        checkRateLimit(client, "wa-global", 2000),
      ]);
      const rejected = limits.find((x) => !x.allowed);
      if (rejected) {
        return NextResponse.json(
    ...
    console.error("WhatsApp inbound processing failed after durable enqueue:", error);
