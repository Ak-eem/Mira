...
export function ReplyForm({
  ...
  // Clear the textarea after a successful send, but leave it untouched on
  // error so the operator doesn't lose what they typed.
  useEffect(() => {
    if (state.success) {
    ...
    <form ref={formRef} action={formAction} className="mt-6 flex flex-col gap-2">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          type="submit"
          disabled={pending}
          className="flex-shrink-0 rounded bg-accent px-4 py-1.5 text-sm font-medium text-white disabled:opacity-50"
        >
