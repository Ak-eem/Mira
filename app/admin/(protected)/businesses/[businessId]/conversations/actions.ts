...
//
// The update is conditioned on claimed_by IS NULL so two operators
// clicking "Take over" at the same moment can't both win -- whichever
// request's update actually affects a row is the one that gets to post
// the system notice and log the activity; the loser's update affects
// zero rows and the page simply re-renders showing who won.
export async function takeOverConversation(businessId: string, conversationId: string): Promise<void> {
  ...
  if (!claimed) {
    // Someone else already claimed it first -- nothing more to do here.
    revalidatePath(`/admin/businesses/${businessId}/conversations/${conversationId}`);
    revalidatePath(`/admin/businesses/${businessId}/conversations`);
    return;
  }
