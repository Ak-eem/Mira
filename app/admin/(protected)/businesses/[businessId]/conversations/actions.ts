...
// claimed_by check in app/api/chat/route.ts and
// app/api/webhooks/whatsapp/route.ts) and a system-notice message marks
// the handover in the thread itself, so anyone reading the transcript
// later can see exactly when and by whom a human stepped in.
...
// mechanism already used for idle-timeout closes, so a customer
// messaging again afterward transparently starts a fresh conversation
// rather than needing any special "reopen" handling.
