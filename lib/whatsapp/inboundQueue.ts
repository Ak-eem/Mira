...
export async function markInboundSent(client: Client, id: string): Promise<void> { const { error } = await client.from("whatsapp_inbound_queue").update({ status: "done", reply_sent_at: new Date().toISOString(), locked_at: null, last_error: null }).eq("id", id); if (error) throw error; }
