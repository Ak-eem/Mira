...
export type ExtractedItem = { kind: ScrapeKind; payload: Record<string, unknown> };
...
function validateItem(value: unknown, allowedImageUrls: Set<string>): ExtractedItem | null {
  ...
  if (value.kind === "product" || value.kind === "service") {
    ...
    const candidateImageUrl = typeof payload.candidate_image_url === "string" && allowedImageUrls.has(payload.candidate_image_url)
      ...
      : null;
    return { kind: value.kind, payload: { name, description, price, ...(value.kind === "product" ? { candidate_image_url: candidateImageUrl } : {}) } };
  }
  if (value.kind === "policy") {
    const title = text(payload.title, 300);
    const content = text(payload.content, 8_000);
    return title && content ? { kind: "policy", payload: { title, content } } : null;
  }
  if (value.kind === "faq") {
    const question = text(payload.question, 500);
    const answer = text(payload.answer, 8_000);
    return question && answer ? { kind: "faq", payload: { question, answer } } : null;
  }
  ...
  return dayOfWeek !== null && (opensAt !== null || payload.opens_at === null) && (closesAt !== null || payload.closes_at === null)
    ? { kind: "business_hours", payload: { day_of_week: dayOfWeek, opens_at: opensAt, closes_at: closesAt } }
    : null;
}
...
export async function extractBusinessInfo(scrape: ScrapeResult): Promise<ExtractedItem[]> {
  ...
  const data = await geminiFetchJson(process.env.LLM_API_KEY ?? "", {
  ...
  return items.map((item) => validateItem(item, allowedImageUrls)).filter((item): item is ExtractedItem => item !== null).slice(0, MAX_ITEMS);
}
